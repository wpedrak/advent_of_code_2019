...
class Line:
    ...
    def is_point(self):
        return self.is_horizontal() and self.is_vertical()

    def has_point(self, point):
        if self.is_horizontal():
            hy = self.begin.y
            return hy == point.y and self.begin.x <= point.x <= self.end.x

        if self.is_vertical():
            vx = self.begin.x
            return vx == point.x and self.begin.y <= point.y <= self.end.y

        raise Exception("has point")

    def __or__(self, line):
        if self.is_point():
            if not line.has_point(self.begin):
                return None
            return self.begin

        if line.is_point():
            if not self.has_point(line.begin):
                return None
            return line.begin
        ...
        v = self if self.is_vertical() else line
        h = self if self.is_horizontal() else line
        ...
        if vx < h.begin.x or vx > h.end.x:
            return None
        if hy < v.begin.y or hy > v.end.y:
            return None
...
def in_to_lines(inp):
    x, y = 0, 0
    lines = []
    for direction in inp.split(','):
        point_from = Point(x, y)
        d = direction[0]
        n = int(direction[1:])
        if d == UP:
            y += n
        ...
        point_to = Point(x, y)
        lines.append(Line(point_from, point_to))

    return lines


def get_lengths(inp, crossings):
    pseudolines = [Line(p, p) for p in crossings]
        ...
        for pl in pseudolines:
            if pl | line:
                ...
                if line.is_horizontal():
                    ...
                    distances[(pl.begin.x, pl.begin.y)] = dist + \
                        abs(point_from.y - pl.begin.y)
        dist += n

    return distances


lines1 = in_to_lines(in1)
lines2 = in_to_lines(in2)
...
lengths_of_crossings1 = get_lengths(in1, crossings)
lengths_of_crossings2 = get_lengths(in2, crossings)

# print(lengths_of_crossings1)
# print(lengths_of_crossings2)


for length in lengths_of_crossings1:
    wire_lengths.append(
        lengths_of_crossings1[length] + lengths_of_crossings2[length])

# print(wire_lengths)

wire_min = min(filter(
    lambda x: x > 0, wire_lengths
))
