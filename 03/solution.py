...
class Line:
    ...
    def __init__(self, p1, p2):
        if p1.x < p2.x:
            ...
            raise Exception("same start and end of Point")
    ...
    def horizontal_intersection(self, line):
        if self.begin.y != line.begin.y:
            return []
        y = self.begin.y
        return [Point(x, y) for x in range(max(self.begin.x, line.begin.x), min(self.end.x, line.end.x))]

    def vertical_intersection(self, line):
        if self.begin.x != line.begin.x:
            return []
        x = self.begin.x
        return [Point(x, y) for y in range(max(self.begin.y, line.begin.y), min(self.end.y, line.end.y))]

    def __or__(self, line):

        if self.is_horizontal() and line.is_horizontal():
            # print(self.horizontal_intersection(line))
            # return self.horizontal_intersection(line)
            return []

        if self.is_vertical() and line.is_vertical():
            # print(self.is_vertical)
            # print(self.is_vertical)
            # print("not sure-vertical")
...
for line1 in lines1:
    for line2 in lines2:
        crossings += line1 | line2

dists = [p.manhatan() for p in crossings]
min_dist = min(filter(lambda x: x != 0, dists))
print(min_dist)
